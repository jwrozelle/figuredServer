...
h_dist.ply <- readOGR(paste(geoDirectory, "/hdist.shp", sep =""), layer = "hdist")
    ...
    if (input$level == 0) {
      ...
      indicator.lflt <- leaflet() %>%
        ...
        addCircleMarkers(lng=comm.pnt_f2$x,
          ...
          bydist$label <- paste("<p> District:", bydist$district, "</p>",
                                "<p> Red MUAC:", bydist$redmuac, "</p>",
                                "<p> Yellow MUAC:", bydist$yellowmuac, "</p>")
          ...
          indicator.lflt <- leaflet() %>%
            ...
            addPolygons(data = h_dist.ply,
                        ...
                        label = lapply(choropleth[["label"]], HTML),
